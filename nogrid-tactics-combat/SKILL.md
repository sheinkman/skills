...
description: Use this skill to build turn-based tactical combat with NO grid — free positioning in continuous 3D space — for a Unity 6 CRPG on the Anime 5E ruleset. Covers movement as NavMesh path-length budgets, world-space range/area targeting, raycast line-of-sight and cover, elevation/height, initiative and action economy, the action-resolution pipeline, status effects, enemy AI, combat forecast UI, encounter setup, and save/load. Also covers architecting the rules as a deterministic, engine-independent C# core (own `.asmdef`, no `UnityEngine`) so combat is unit-testable, AI can simulate outcomes, and state is save/replay-able. Trigger on tactics/tactical combat, turn-based, initiative, action economy, action points, movement range, line of sight, cover, area of effect, ability resolution, damage roll, enemy AI, NavMesh movement budget, ActionResult, combat forecast, "XCOM-like", "Fire Emblem-like", adapting grid/tile tactics tutorials to free movement, and also combat architecture, engine-independent/testable rules, determinism, seeded RNG, Edit Mode tests, and combat save/load. Targets Unity 6 + URP. Load `references/` files on demand.
---
